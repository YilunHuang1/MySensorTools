...
| `uwb/smoke_test/sensor_tools`, `requirements.txt` | Copy-folder deployment includes runtime modules; one pip requirements installation; no project install/build or .deps loader | S: copied-directory execution without site packages; embedded-source consistency; live result in migration status |
