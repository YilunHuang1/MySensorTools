#!/usr/bin/env python3
"""
UWB BLE 测距主程序（Standalone 版本，无 ROS2 依赖）
用于机器狗 UWB 锚点与 iPhone Qorvo Nearby Interaction App 连接测距。

Usage:
    sudo python3 run_uwb_ble.py --port /dev/ttyUSB0
    sudo python3 run_uwb_ble.py --port /dev/ttyS7
"""
import sys
import os
import argparse
...
class Characteristic(dbus.service.Object):
    ...
    def WriteValue(self, value, options):
        ...
        print("📥 Received from Central (iPhone):")
        print('   ' + ' '.join(f'{b:02X}' for b in rcv_data))

        if rcv_data[0] == 0x0A:
            # iPhone 发送 initialize 命令
            if tx_char_global and tx_char_global.notifying:
                configureDataCmd = CmdBuilder.build(CmdEnum.UWB_CONFIGURE_DATA)
                print("📤 Notify UWB_CONFIGURE_DATA to iPhone:")
        ...
        if rcv_data[0] == 0x0B:
            # iPhone 发送 configureAndStart 命令
    ...
    def StartNotify(self):
        self.notifying = True
        print("✅ Central (iPhone) subscribed to TX notifications")

    @dbus.service.method("org.bluez.GattCharacteristic1", in_signature="", out_signature="")
    def StopNotify(self):
        self.notifying = False
        print("🛑 Central (iPhone) unsubscribed from TX notifications")
...
def main():
    global serialHandler, mainloop

    parser = argparse.ArgumentParser(description='UWB BLE Ranging - Connect with iPhone Qorvo App')
    parser.add_argument('--port', type=str, default='/dev/ttyS7',
                        help='Serial port for UWB module (default: /dev/ttyS7)')
    ...
    print("=" * 60)
    print("  UWB BLE Ranging - Qorvo Nearby Interaction")
    print("=" * 60)
    print(f"  Serial port      : {args.port}")
    print(f"  Baudrate         : {args.baudrate}")
    print(f"  BLE name         : vita_0_01")
    print(f"  CSV logging      : {'ON' if enable_csv else 'OFF'}")
