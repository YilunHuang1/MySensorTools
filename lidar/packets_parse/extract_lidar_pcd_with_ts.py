...
支持三种时间戳模式（可在配置区切换）：
  ...
  MODE C: "new_driver" —— 对照实现新驱动逻辑（v2.2.7+ / commit e914041）
...
import struct
import math
from sensor_tools.lidar import iter_packet_messages, ResolutionTracker
import os
import sys
import time
import calendar
...
# "new_driver"      : 对照实现新驱动逻辑（推荐，与 SLAM 日志一致）
...
def parse_pkt_lidar_ts(pkt: bytes) -> Optional[float]:
    ...
    try:
        ...
        t = dt.datetime(year, month, day, hour, minute, second, tzinfo=dt.timezone.utc).timestamp()

        # 微秒字段（小端 uint32）
        microseconds = struct.unpack_from('<I', pkt, 12)[0]
        if microseconds >= 1_000_000:
            return None
        usec = microseconds * 1e-6
...
def decode_point_cloud_packet_with_ts(
    ...
    points = []
    last_firing_channel = 15
    while last_firing_channel > 0 and struct.unpack_from('<H', pkt, 18 + last_firing_channel * 3)[0] == 0:
        last_firing_channel -= 1
    for chan in range(16):
        ...
        if ts_mode == "driver_original":
            ...
            ts = float(pkt_lidar_ts - INTRA_BLOCK_OFFSET[max(last_firing_channel - chan, 0)])
        else:
            # per_packet 简单方案：
        ...
        elif INVALID_POINTS != 'drop':
            coordinate = float('nan') if INVALID_POINTS == 'nan' else 0.0
            points.append(PointWithTs(coordinate, coordinate, coordinate, 0., ts))
...
def save_pcd_with_timestamp(filepath: str, points: List[PointWithTs], binary: bool = True):
    # Preserve arrival order: sorting would hide regressions and disagree with CSV boundaries.
...
class FrameTimestampStats:
    ...
    seq_loss_count: int = 0       # 本帧内 sequence_num 跳变次数（序号跳变）
...
def print_frame_stats(stats_list: List[FrameTimestampStats]):
    ...
    print(f"{'帧':>4} {'first_ts':>18} {'时长(ms)':>10} {'包数':>5} {'序号跳变':>8} {'正向缺号估计':>6} {'点数':>6}")
    print(f"{'-'*80}")
    ...
    overlaps = [(a.frame_idx, b.frame_idx, (b.first_point_ts - a.last_point_ts) * 1e6)
                for a, b in zip(stats_list, stats_list[1:]) if b.first_point_ts < a.last_point_ts]
    print(f"帧间时间回退（后帧首点 < 前帧末点）: {len(overlaps)}")
    for a, b, gap_us in overlaps[:10]:
        print(f"  frame {a}->{b}: gap={gap_us:.3f} us")
    ...
    print(f"\n序号连续性统计:")
    print(f"  序号不连续次数: {total_loss_events}  正向缺号估计: {total_loss_pkts}")
    print(f"  存在缺号的帧:   {frames_with_loss} / {len(stats_list)} 帧")
    if low_pkt_frames:
...
def main():
    ...
    # new_driver 模式额外状态（对照实现 C++ decodeMsopPktSerialPointCloud）
    ...
    for message in iter_packet_messages(MCAP_FILE, TOPIC):
        ...
        for pkt_type, pkt_data in sub_packets:
            ...
            if prev_seq_num >= 0:
                ...
                if seq_num != expected:
                    delta = (seq_num - prev_seq_num) % 65536
                    loss = delta - 1 if 1 < delta < 32768 else 0  # duplicates/resets are not 65535 lost packets
            ...
            # ============================================================
            # new_driver 模式: 对照实现 C++ decodeMsopPktSerialPointCloud
            # ============================================================
            if TIMESTAMP_MODE == "new_driver":
                ...
                resolution = resolution_tracker.update(resolution)
                ...
                if abnormal_split:
                    ...
                    if first_frame_done and len(frame_points) > 0:
                        ...
                        pcd_name = f"frame_{frame_count:04d}_{first_ts:.6f}.pcd" if SAVE_PCD else ""
                        if SAVE_PCD:
                            save_pcd_with_timestamp(os.path.join(out_dir, pcd_name), frame_points, binary=OUTPUT_BINARY)
                        stats = FrameTimestampStats(
                            frame_idx=frame_count, pcd_filename=pcd_name, first_point_ts=first_ts,
                            last_point_ts=last_ts, duration_ms=(last_ts - first_ts) * 1000,
                            point_count=len(frame_points), ts_mode=TIMESTAMP_MODE,
                            pkt_count=len(frame_pkts_buffer), seq_loss_count=frame_seq_loss_count,
                            seq_loss_pkts=frame_seq_loss_pkts, abnormal_split=True,
                        )
                        frame_stats.append(stats)
                        loss_str = f" 正向缺号:{frame_seq_loss_pkts}" if frame_seq_loss_pkts > 0 else ""
                        ...
                        print(f"  帧 {frame_count:4d}: {len(frame_points):6d} 点 | "
                              ...
                              f"{pkt_warn}{loss_str} ↩️角度回跳分帧" + ("" if SAVE_PCD else " [未保存PCD]"))
                        frame_pkt_count = 0
                ...
                if azimuth_trans < resolution:
                    ...
                    if ((pre_hor_angle_ + resolution) % 36000) < resolution:
                        ...
                        if first_frame_done and len(frame_points) > 0:
                            ...
                            print(f"  帧 {frame_count:4d}: {len(frame_points):6d} 点 | "
                                  f"first={first_ts:.6f} last={last_ts:.6f} dur={stats.duration_ms:.1f}ms"
                                  f"{pkt_warn}{loss_str}" + ("" if SAVE_PCD else " [未保存PCD]"))
                            frame_pkt_count = 0
                            frame_seq_loss_count = 0
                            frame_seq_loss_pkts = 0
                            frame_points = []
                ...
                if not first_frame_done:
                    ...
                    if len(frame_points) > 0:
                        ...
                        stats = FrameTimestampStats(
                            ...
                            pcd_filename=pcd_name if SAVE_PCD else "",
                            ...
                            pkt_count=len(frame_pkts_buffer),
    ...
    if frame_points and (MAX_FRAMES is None or frame_count < MAX_FRAMES):
        ...
        if SAVE_PCD:
            save_pcd_with_timestamp(pcd_path, frame_points, binary=OUTPUT_BINARY)
        frame_stats.append(FrameTimestampStats(frame_count, pcd_name if SAVE_PCD else "", first_ts, last_ts,
                                               (last_ts - first_ts) * 1000, len(frame_points), TIMESTAMP_MODE, pkt_count=len(frame_pkts_buffer), partial=True))
    ...
    if not frame_stats:
        raise SystemExit("error: no valid LiDAR frames in selected range")

    # 保存 CSV 统计
    csv_path = os.path.join(out_dir, f"frame_timestamps_{TIMESTAMP_MODE}.csv")
    with open(csv_path, 'w') as f:
        f.write("frame_idx,first_point_ts,last_point_ts,duration_ms,point_count,pkt_count,seq_loss_count,seq_loss_pkts,abnormal_split,partial,pcd_file\n")
        for s in frame_stats:
            f.write(f"{s.frame_idx},{s.first_point_ts:.9f},{s.last_point_ts:.9f},"
                    ...
                    f"{s.seq_loss_count},{s.seq_loss_pkts},{int(s.abnormal_split)},{int(s.partial)},{s.pcd_filename}\n")
    print(f"\n帧时间戳统计已保存: {csv_path}")
    print("\nfirst/last 是输出点顺序的首末时间，partial=1 表示末尾不完整帧。")
    print("与在线驱动比较时需核对时间源、量程过滤、无效点策略、标定和部署版本；不保证默认配置等价。")
    print("seq_loss_count 为序号不连续次数；seq_loss_pkts 仅估计正向缺号，不能证明传感器或 UDP 丢包。")



def cli():
    global MCAP_FILE, TOPIC, OUTPUT_DIR, ANGLE_CSV, TIMESTAMP_MODE, TIMESTAMP_RANGE, SAVE_PCD, OUTPUT_BINARY, MAX_FRAMES, INVALID_POINTS
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('mcap')
    parser.add_argument('--topic', default='/lidar_packets')
    parser.add_argument('--output-dir', default='pcd_output_with_ts')
    parser.add_argument('--calibration', default=ANGLE_CSV)
    parser.add_argument('--mode', choices=['new_driver', 'per_packet', 'driver_original'], default='new_driver')
    parser.add_argument('--start', type=float, help='LiDAR source Unix seconds, inclusive')
    parser.add_argument('--end', type=float, help='LiDAR source Unix seconds, inclusive')
    parser.add_argument('--save-pcd', action='store_true')
    parser.add_argument('--binary', action='store_true')
    parser.add_argument('--max-frames', type=int)
    parser.add_argument('--invalid-points', choices=['nan', 'zero', 'drop'], default='nan')
    args = parser.parse_args()
    if (args.start is None) != (args.end is None) or (args.start is not None and args.end < args.start):
        parser.error('provide an ordered --start/--end pair')
    if args.max_frames is not None and args.max_frames < 1:
        parser.error('--max-frames must be positive')
    MCAP_FILE, TOPIC, OUTPUT_DIR, ANGLE_CSV = args.mcap, args.topic, args.output_dir, args.calibration
    TIMESTAMP_MODE = args.mode
    TIMESTAMP_RANGE = None if args.start is None else (args.start, args.end)
    SAVE_PCD, OUTPUT_BINARY, MAX_FRAMES, INVALID_POINTS = args.save_pcd, args.binary, args.max_frames, args.invalid_points
    return main()
